...
int main(int argc, char* argv[])
{
	...
	uint32_t *d_bins = (uint32_t*) AllocOnDevice(HISTO_HEIGHT * HISTO_WIDTH * sizeof(uint32_t));

	uint32_t *temp_bins = (uint32_t*) malloc(HISTO_HEIGHT * HISTO_WIDTH*sizeof(uint32_t));

	uint32_t *d_input = (uint32_t*) CopyInputToDevice(input, INPUT_HEIGHT, INPUT_WIDTH);
    ...
    TIME_IT("opt_2dhisto",
           1000,
           opt_2dhisto(d_input, INPUT_WIDTH, INPUT_HEIGHT, d_bins);)

    /* Include your teardown code below (temporary variables, function calls, etc.) */

	
	CopyHistToHost(d_bins, temp_bins, HISTO_HEIGHT*HISTO_WIDTH*sizeof(uint));
	
	for(int i=0; i < HISTO_HEIGHT*HISTO_WIDTH;i++)
	{
		kernel_bins[i] = (uint8_t) (temp_bins[i] < UINT8_MAX? temp_bins[i]: UINT8_MAX);
		//For debug
		//if(temp_bins[i]!=0)
		//	printf("%d: %d\n", i, temp_bins[i]);
	}
